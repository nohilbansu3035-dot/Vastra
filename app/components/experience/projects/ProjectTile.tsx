...
const ProjectTile = ({ project, index, position, rotation, activeId, onClick }: ProjectTileProps) => {
  ...
  const handleTileClick = (e: ThreeEvent<MouseEvent>) => {
    e.stopPropagation();
    onClick(); // Call the parent onClick if needed

    if (!project.url && !project.id) return;
    
    // Animate the entire tile when clicked
    if (projectRef.current) {
        gsap.to(projectRef.current.position, { z: 0.1, duration: 0.1 })
          .then(() => gsap.to(projectRef.current?.position || {}, { z: 0.4, duration: 0.3 }));
    }
  ...
  return (
    <group
      ...
      onPointerOver={() => {
        ...
        if (isProjectSectionActive) document.body.style.cursor = 'pointer';
      }}
      onPointerOut={() => {
        if (!isMobile && isProjectSectionActive) setHovered(false);
        if (isProjectSectionActive) document.body.style.cursor = 'auto';
      }}>
